...
compareResult :: String -> String -> [Level] -> Int -> Int -> [(String, String, String)] -> IO ()
compareResult ui packName levels idx currentScore pastAttempts = do
  ...
  case cleanExpressionParser ui of
    ...
    Right parsedTree -> do
      ...
      let allowedNums = nums currentLevel

      let usedOps = extractOps parsedTree
      let allowedOps = ops currentLevel

      -- Error accumulation
      let numErrors =
            (["You can only use numbers in this list, as many times as they appear: " ++ show allowedNums | not (null (usedNums \\ allowedNums))])
        ...
        then do
          ...
          case eval parsedTree of
            ...
            Left mathError -> do
              ...
              displayAllErrors [e3]
              enterExpr packName levels idx currentScore newAttempts

            -- Math is correct, now check if it is the right answer
            Right finalAnswer -> do
              if finalAnswer == target currentLevel
                then do
                  let numCount = length usedNums
                  let maxNums = length allowedNums
                  let scoreIncrease = maxNums - numCount + 1
                  let newScore = currentScore + scoreIncrease

                  putStrLn ("Success! " ++ show (target currentLevel))
                  putStrLn ("You used " ++ show numCount ++ "/" ++ show maxNums ++ " numbers. Score increased by: " ++ show scoreIncrease ++ ". New score: " ++ show newScore)
                  putStrLn "Type :new to go to the next level."

                  -- add to list of attempts
                  let successMsg = "Evaluated to target: " ++ show finalAnswer
                  let newAttempts = pastAttempts ++ [(ui, "SUCCESS", successMsg)]

                  enterExpr packName levels idx newScore newAttempts
                else do
                  let e4 = "Not quite. " ++ show finalAnswer ++ " is not the target number."
                  let newAttempts = pastAttempts ++ [(ui, "UNSUCCESSFUL - Expression didn't evaluate to target", e4)]
                  displayAllErrors [e4]
                  enterExpr packName levels idx currentScore newAttempts

-- Prompts the user for their next move and captures it
enterExpr :: String -> [Level] -> Int -> Int -> [(String, String, String)] -> IO ()
enterExpr packName levels idx currentScore pastAttempts = do
  putStr "> "
  hFlush stdout
  ui <- getLine
  checkInput ui packName levels idx currentScore pastAttempts

-- Display level pack information when a pack is chosen
printPackDetails :: [Level] -> String -> IO ()
printPackDetails loadedLevels packName =
  if null loadedLevels
    then do
      putStrLn $ "Error: Could not load file " ++ packName
      mainLoop
  else do
    putStrLn $ "        " ++ packName ++ "       "
    putStrLn "   Type :h for commands   "

    -- don't save if it is the random pack
    progressTracker <- if packName == "Random Pack" then
      return "1"
    else catchIOError (readFile ("progress_" ++ packName))
             (\e -> if isDoesNotExistError e then return "1" else ioError e)

    let levelNo = read progressTracker :: Int
    let idx = levelNo - 1

    let firstLevel = getNthLevel levelNo loadedLevels
    printLevelDetails packName loadedLevels idx levelNo firstLevel
    enterExpr packName loadedLevels idx (levelNo - 1) []
    ...
    then do
      ...
      mainLoop
        ...
        then do
          ...
          putStrLn "--------------------------"
          printPackDetails loadedLevels filename
...
checkInput :: String -> String -> [Level] -> Int -> Int -> [(String, String, String)] -> IO ()
checkInput ui packName levels idx currentScore pastAttempts
  | ui == ":q" || ui == ":quit" = do
    if packName /= "Random Pack"
        then do
          let saveFile = "progress_" ++ packName
          writeFile saveFile (show (idx + 1))
          putStrLn $ "Progress written to " ++ saveFile
        else
          putStrLn "Random pack progress not saved."
  ...
  | otherwise = compareResult ui packName levels idx currentScore pastAttempts
